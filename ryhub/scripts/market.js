// Loader delay + animasi smooth masuk
window.addEventListener("load", () => {
  ...
  // Fungsi untuk mengambil data harga cryptocurrency
async function fetchCryptoPrices() {
    try {
      const response = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum&vs_currencies=usd');
      const data = await response.json();
      // Menampilkan harga di konsol
  ...
  // Panggil fungsi untuk mengambil dan menampilkan harga
  fetchCryptoPrices();
              ...
              .then(data => {
                if (data.price) {
                  ...
                  fetch(`https://finnhub.io/api/v1/quote?symbol=${symbol}&token=${finnhubKey}`)
                    ...
                    .catch(() => {
                      document.getElementById(symbol.toLowerCase()).innerHTML = `${symbol}: ❌ Gagal dari semua API`;
                    });
                }
              });
          }
